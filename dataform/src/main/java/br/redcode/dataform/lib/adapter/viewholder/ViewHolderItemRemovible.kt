...
class ViewHolderItemRemovible(itemView: View) : ViewHolderGeneric<Spinnable>(itemView) {
    ...
    fun popular(duasLinhas: Spinnable, myOnItemClickListener: ((Int) -> Unit)?, configuracao: QuestionSettings) {
        popular(duasLinhas, myOnItemClickListener)
        imageViewRemoverItem.visibility = if (configuracao.editable) View.VISIBLE else View.GONE
    }
